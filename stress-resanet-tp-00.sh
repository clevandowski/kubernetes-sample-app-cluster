...
#VILLES_SERVICE_ENDPOINT=http://192.168.99.100:30745
RESANET_SERVICE_ENDPOINT=$1
...
launch_request() {
  ...
  if timeout 3s curl -v -f --header 'Accept: application/json' $RESANET_SERVICE_ENDPOINT/villes/$(( ( RANDOM % 4 )  + 1 )) >> logs/curl-responses.$$.log 2>> logs/curl-infos.$$.log; then
...
#    (time (curl GET --header 'Accept: application/json' $RESANET_SERVICE_ENDPOINT/villes/$(( ( RANDOM % 4 )  + 1 )) 2>/dev/null) | grep "^real") &
    (date +%Y-%m-%d\ %H:%M:%S.%N && time launch_request &) 1>> logs/time-output.$$.log 2>> logs/time-output.$$.log
    sleep .1
  done
}

# echo "RESANET_SERVICE_ENDPOINT: $RESANET_SERVICE_ENDPOINT"
